package Ch13_1;


//명품자바 상속 : https://cs-ssupport.tistory.com/84



// 01 일반클래스 상속관계
class Super1{
	void Func() {} //내용은 없지만 구성은 있으므로 완성형
	
}
class Son1 extends Super1{
	void Func() {System.out.println("Son1's Function()");}
}


// 02 추상클래스 상속관계
abstract class Super2{
	abstract void Func();
	
}
class Son2 extends Super2{

	@Override
	void Func() {
		System.out.println("Son2's Function()");
		
	}
	
	
}


public class C01AbstractMain {

	public static void main(String[] args) {
		// 01 일반클래스 상속관계
//		Super1 ob1 = new Super1(); // 상위 클래스로 객체 생성 o
//		Son1 ob2 = new Son1();     // 하위 클래스로 객체 생성 o
//		ob1.Func();				   // 상위 클래스에서 하위 클래스가 추가한 메서드 접근 x
//								   // 상위 클래스에서 정의한 메서드를 하위클래스에서 재정의한 경우에는 접근 가능 o
//		
//		Super1 ob3 = ob2;          //업캐스팅
//		ob3.Func();
		
		
		// 02 추상클래스 상속관계
//		Super2 ob1 = new Super2(); // 추상클래스이기 때문에 완성시키지 않아서 오류남
		Son2 ob2 = new Son2();     // 가능
		
		Super2 ob3 = ob2;		   // Upcasting 
		ob3.Func();
		
	}


}
